    ...
    UUPSUpgradeable
{
    ...
    mapping(address => uint) public pendingRevAmount;
    ...
    IERC20Rebasing public constant RETH =
    ...
    function initialize(
        ...
        RETH.configure(YieldMode.CLAIMABLE);
    ...
    function updatePythContract(address _pythContract) public onlyOwner {
        pyth = IPyth(_pythContract);
    }

    function updatePriceFeed(bytes32 _priceFeed) public onlyOwner {
        priceFeed = _priceFeed;
    }
    ...
    function settleAuction(
        SettleAuctionParams calldata settleAuctionParams
    )
        public
        payable
        atStageSolutionSubmission(settleAuctionParams.auctionId)
        returns (bytes32 clearingOrder)
    {
        ...
        ) = auctionData[settleAuctionParams.auctionId]
                .initialAuctionOrder
                .decodeOrder();
        uint256 currentBidSum = auctionData[settleAuctionParams.auctionId]
            .interimSumBidAmount;
        bytes32 currentOrder = auctionData[settleAuctionParams.auctionId]
            .interimOrder;
        ...
        do {
            bytes32 nextOrder = sellOrders[settleAuctionParams.auctionId].next(
                currentOrder
            );
        ...
        if (
            auctionData[settleAuctionParams.auctionId].minFundingThreshold >
            currentBidSum
        ) {
            auctionData[settleAuctionParams.auctionId]
        ...
        auctionData[settleAuctionParams.auctionId].interimOrder = bytes32(0);
        auctionData[settleAuctionParams.auctionId]
    ...
    function processFeesAndAuctioneerFunds(
        SettleAuctionParams memory settleAuctionParams,
        ...
        if (
            ...
            address biddingTokenAddress = address(
                auctionData[settleAuctionParams.auctionId].biddingToken
            );
            ...
            uint256 usdValue = convertToUSD(
                ...
                biddingTokenAmount,
                usdFeeAmount,
                settleAuctionParams.pythUpdateData
            );
            ...
            if (biddingTokenAddress != uniswapV2Router.WETH()) {
                uint256 initialEthBalance = address(this).balance;
                swapTokensForEth(biddingTokenAddress, feeAmount);
            ...
            if (ethAmount > 0) {
                ...
                emit Distribution(settleAuctionParams.auctionId, ethAmount);
            }
        }
    }
    ...
    function convertToUSD(
        ...
        if (tokenAddress == address(uniswapV2Router.WETH())) {
            ...
            PythStructs.Price memory currentETHPrice = pyth.getPrice(priceFeed);
            uint256 ethPrice = convertToUint(currentETHPrice, 18);
    ...
    // TODO: we should probably move something like this into the a library
    ...
    function swapTokensForEth(address token, uint256 tokenAmount) private {
        ...
        {} catch {
            pendingRevAmount[token] += tokenAmount;
        }
    }

    //sends unswapped tokens to multisig, so they can be swapped to eth manually for revshare
    function transferPendingRevAmount(address token) public onlyOwner {
        IERC20(token).transfer(msg.sender, pendingRevAmount[token]);
    }
    ...
    function claimYieldAndGas() public {
        uint256 ethBalance = address(this).balance;
        address feeReceiver = registeredUsers.getAddressAt(feeReceiverUserId);
        RETH.claim(address(this), RETH.getClaimableAmount(address(this)));
        uint256 usdbAmount = USDB.claim(
            address(this),
            USDB.getClaimableAmount(address(this))
        );
        if (usdbAmount > 0) {
            swapTokensForEth(address(USDB), usdbAmount);
        }
        //claim 50% of the gas so we get it immediately
        BLAST.claimAllGas(address(this), address(this));
        uint256 ethAmount = address(this).balance.sub(ethBalance);
        (bool success, ) = feeReceiver.call{value: ethAmount}("");
        if (!success) revert EthSendingFailed();
        emit GasAndFeeClaimed(ethAmount);
    }
}
