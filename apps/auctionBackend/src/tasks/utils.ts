...
import { ETH_PRICE_FEED_ID } from "shared/constants";
...
export async function getUniswapV2RouterAddress(
  ...
  } else if (chainId === 421613 || chainId === 168587773) {
...
export async function getOracleAddress(
  ...
  if (chainId == 1 || chainId == 31337) {
    ...
    return "0xA2aa501b19aff244D90cc15a4Cf739D2725B5729";
  } else {
...
export async function getPriceFeedAddress(
  hre: HardhatRuntimeEnvironment,
): Promise<string> {
  let oracleAddress = "";
  const chainId = (await hre.ethers.provider.getNetwork()).chainId;
  if (chainId === 168587773) {
    return ETH_PRICE_FEED_ID;
  } else {
